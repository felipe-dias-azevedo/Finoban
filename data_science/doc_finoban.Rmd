...
```{r}
df <- data.frame(idade,regiao.r,valorImovel,renda,tempoFinanciamento,`%rendaFinanciamento`, banco.r,avaliacao.r)
...
```{r}
#tm <- as.POSIXct("25072013 08:32:07", format = "%d%m%Y %H:%M:%S")
#tm <- Sys.time()
#tm <- sample(seq(Sys.Time('2021/01/01 00:00:00 -03'), Sys.time(), by="day"), 10)
#tm <- sample(seq(as.Date('2021/01/01'), as.Date('2021/06/01'), by="day"), 10)
tm
# tm <- rdate <- function(x,
...
#random_datetime(10, st = "2015/01/01", et = "2015/12/31")
